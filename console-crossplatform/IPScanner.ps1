...
function Get-HostInfo {
    ...
    if ($IsWindows) {
        ...
        # Find Gateway
        $global:gateway = (netstat -rn | Select-String -Pattern '^default' | ForEach-Object { $_.Line -split '\s+' | Where-Object { $_ } }[1])
        $gatewayParts = $gateway -split '\.'
        $global:gatewayPrefix = "$($gatewayParts[0]).$($gatewayParts[1]).$($gatewayParts[2])."

        # Internal IP (find IP on the default interface)
        $defaultInterface = (netstat -rn | Select-String -Pattern '^default' | ForEach-Object { $_.Line -split '\s+' | Where-Object { $_ } }[3])
        $global:internalIP = (ifconfig $defaultInterface | Select-String -Pattern 'inet ' | ForEach-Object { $_.Line.Trim().Split()[1] })

        # Host adapter type (interface name)
        $global:adapter = $defaultInterface

        # My Mac
        $global:myMac = (ifconfig $defaultInterface | Select-String -Pattern 'ether' | ForEach-Object { $_.Line.Trim().Split()[1] })

        # Subnet mask
        $subnetMaskLine = (ifconfig $defaultInterface | Select-String -Pattern 'netmask')
        $subnetHex = $subnetMaskLine.Line.Trim().Split()[3]  # e.g., 0xffffff00
        $subnetMask = [string]::Join('.', (0..3 | ForEach-Object { [Convert]::ToInt32($subnetHex.Substring($_*2 + 2, 2), 16) }))

        # Domain (from scutil or hostname)
        $domain = (scutil --get LocalHostName)  # Or use hostname -d if available
        if ([string]::IsNullOrEmpty($domain)) { $domain = 'Unknown' }
    } else {
